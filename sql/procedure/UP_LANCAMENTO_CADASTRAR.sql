...
SELECT	LTRIM(RTRIM(ISNULL(@DESCRICAO, ''))), @FORNECEDOR_ID, @CATEGORIA_ID, @CENTRO_CUSTO_ID
		, @CONTA_BANCARIA_ID, @CLIENTE_ID, @FECHAMENTO_ID, @DATA_LANCAMENTO, @CUSTO_FIXO, (@VALOR*TL.MULTIPLICADOR)
FROM	fin.CATEGORIA C
INNER	JOIN fin.TIPO_LANCAMENTO TL ON TL.ID = C.TIPO_LANCAMENTO_ID
